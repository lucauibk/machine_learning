...
We split the data into 80\% training ($31{,}997$ samples) and 20\% validation ($8{,}000$ samples) using a fixed random seed (\texttt{random\_state=42}) for reproducibility. The preprocessor was fit exclusively on the training split and then applied to the validation set to prevent data leakage.
...
\end{equation}
which corresponds to maximum a-posteriori estimation under a Gaussian prior $p(w) \propto \exp\!\bigl(-\frac{\alpha}{2}\|w\|^2\bigr)$ as derived in Lecture~4. The regularisation parameter $\alpha$ controls the bias-variance trade-off.

We selected $\alpha$ by evaluating 60 logarithmically spaced values in $[10^{-3}, 10^4]$ on the validation set (see Fig.~\ref{fig:ridge}). Ridge is a well-motivated linear baseline: it is fast to train, fully interpretable, and prevents weight explosion when the number of features is large relative to the number of truly informative signals.
...
The hyperparameter search for Ridge Regression (Fig.~\ref{fig:ridge}) shows that the optimal $\alpha = 0.001$ corresponds to nearly no regularisation. Very high values of $\alpha$ produce clear underfitting, while lower values converge to near-ordinary-least-squares behaviour.

For the MLP, the best configuration uses a single hidden layer of $256$ neurons with tanh activations and learning rate $0.01$. The training loss curve (Fig.~\ref{fig:mlp_loss}) decreases rapidly in the first $\sim50$ epochs and then converges smoothly. The gap between training $R^2 = 0.664$ and validation $R^2 = 0.564$ indicates mild overfitting, which could be reduced with weight decay or dropout.

We considered but discarded the \texttt{weather\_description} text columns, as \texttt{weather\_main} already captures the essential information with far fewer features. We also considered k-nearest-neighbour regression, but dismissed it due to the curse of dimensionality in a 100-dimensional space.

Temporal features (hour of day, day of week, month) would likely be the most impactful addition, as power demand has strong time-of-day and seasonal patterns not captured by weather alone.

\section{Conclusion}
\label{sec:con}

We applied two regression methods — Ridge Regression and an MLP Neural Network — to the power consumption prediction task. The MLP achieved a validation RMSE of $3{,}139\,\text{MW}$ and $R^2 = 0.564$, compared to RMSE $= 3{,}950\,\text{MW}$ and $R^2 = 0.310$ for Ridge Regression. The MLP proved substantially more effective, indicating that non-linear interactions between weather features are important for predicting power consumption. The remaining unexplained variance ($R^2 \approx 0.44$) is likely due to the absence of temporal features such as time of day and day of week, which are among the strongest drivers of energy demand. The final model (MLP) is stored in \texttt{models/final\_model.pkl} (0.87\,MB, well below the 50\,MB limit) and loaded at test time without retraining.
